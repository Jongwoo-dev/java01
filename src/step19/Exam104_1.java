 ...
 * - 자원을 사용한 후 매번 자원을 해제시키는 close를 호출하는 것이 번거로웠다.
 *   이런 번거로움을 해결하기 위해 등장한 문법이다.
 * - try 블록을 벗어날 때 자동으로 close()를 호출한다.
 * - 단 그 자원은 java.lang.AutoClosebla 인터페이스 구현체(인터페이스 규칙에 따라 만든 클래스)여야 한다.
 * - 문법 
...
public class Exam104_1 {
  public static void main(String[] args) {
    ...
    try (
      ...
      e.printStackTrace();
    } // try 블록을 벗어날 때 자동으로 close()를 호출한다. 따라서 finally 블록이 필요없다.
