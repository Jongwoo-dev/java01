...
=> 텍스트 파일과 바이너리 파일을 구분하는 방법?
   ...
   - 메모장으로 읽어 들인 후 변경하고 저장했을 때 파일 형식이 깨진다면
     바이너리 파일이다. 
     예) .xls, .doc, .ppt, .fig, .avi, .mp3, .mp4, .jpg, .class
     최근의 MS 오피스 파일(.doxc, .pptx, .xlsx)은 XML 형식으로 저장하기 때문에
