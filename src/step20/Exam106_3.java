 ...
 * - 자바에서 스레드를 관리할 떄는 1에서 10까지 우선순위를 부여하여 
 *   실행 횟수를 조정한다.
 *   기본 값은 5이다.
 ...
 * 1) Round-Robin 방식
 *    - 우선 순위를 고려하지 않는다.
 *    - 10ms ~ 100ms 단위로 돌아가면서 프로세스에게 CPU를 분배한다.
 *    - 대표적인 OS: Windows
 * 2) Priority + Aging 방식
 *    - 우선 순위에 따라 CPU 분배를 조절한다.
 *    - Aging은 우선 순위에서 밀릴 떄 마다 우선 순위를 높이는 기법이다.
