 ...
 *              서버는 스스로 포트 번호를 지정하여 사용한다.
 *              클라이언트는 OS로부터 자동으로 배정 받는다.
 *              포트번호는 중복되어서는 안된다.
 * well-known port : 0 ~ 1023 까지의 포트번호를 말한다.
 *                   기존의 유명 서비스가 주로 사용하는 번호로 일반 프로그램에서 사용하지 말도록 
 *                   권고하고 있다.
 *                   20 - FTP 데이터 전송 포트
 *                   21 - FTP 제어 포트
 *                   22 - SSH(Secure Shell) 서버 = Secure 텔넷 버전
 *                   23 - 텔넷 서버
 *                   25 - SMTP 서버. 메일 발송 서버
 *                   53 - DNS 서버 
 *                   80 - 웹 서버가 사용
...
=> 단점
   - 클라이언트가 연결되면 그 클라이언트의 접속 정보를 계속 서버 메모리에 유지해야 하기 때문에
     메모리를 많이 차지한다.
...
=> 단점
   - 클라이언트의 정보를 유지하기 않기 때문에 중간 작업 결과를 서버에 보관하지 않는다. 
     따라서 영속성 있는 작업을 할 수 있다.
=> 예)
   114 안내, 메신저, 웹(HTTP)
