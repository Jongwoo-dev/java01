 ...
 * - 방법?
 *   => 동시에 실행할 코드를 Thread라는 클래스 블록에 묶어 놓는다.
 *   => 그리고 그 스레드를 실행 한다.
 */
...
public class GameServer {
  ...
  public static void main(String[] args) throws Exception {
    ...
    while (true) {
      ...
      t.start(); // 스레드를 독립적으로 시작 시킨다. 주의! run()을 직접 호출하는 것이 아니다.
    } //while
    //ss.close();
  } //main()
    ...
    public void run() {
      try (
          ...
          PrintStream socketOut = new PrintStream(
              new BufferedOutputStream(socket.getOutputStream()), true,"UTF-8"); /* charset */ ) {
